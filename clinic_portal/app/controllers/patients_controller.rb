class PatientsController < ApplicationController
  ...
  before_action :receptionist_only, except: [:index, :show]
  before_action :set_patient, only: %i[ show edit update destroy ]

  # GET /patients or /patients.json
  def index
    @query = params[:query]
    patients = @query ? Patient.where("name ILIKE ?", "%#{@query}%") : Patient.all
  ...
  private
    ...
    def patient_params
      params.require(:patient).permit(:name, :age, :gender, :address, :phone)
    end
end
